...
class PETRFormatBundle3D(DefaultFormatBundle):
    ...
    def __call__(self, results):
        ...
        for key in ['input_ids', 'vlm_labels','vlm_attn_mask']:
            if key not in results:
                continue
        ...
        if self.with_ego:
            ...
            if 'ego_fut_trajs' in results:
                results['ego_fut_trajs'] = DC(to_tensor(results['ego_fut_trajs'][None, ...]), stack=True)
            if 'path_points_future' in results:
                results['path_points_future'] = DC(to_tensor(results['path_points_future'][None, ...]), stack=True)
            if 'path_future_mask' in results:
                results['path_future_mask'] = DC(to_tensor(results['path_future_mask'][None, None, ...]), stack=True)
            if 'ego_fut_masks' in results:
                results['ego_fut_masks'] = DC(to_tensor(results['ego_fut_masks'][None, None, ...]), stack=True)
            if 'ego_fut_cmd' in results:
                results['ego_fut_cmd'] = DC(to_tensor(results['ego_fut_cmd'][None, None, ...]), stack=True)
            if 'cmd_speed' in results:
                results['cmd_speed'] = DC(to_tensor(results['cmd_speed'][None, None, ...]), stack=True)
            if 'cmd_path' in results:
                results['cmd_path'] = DC(to_tensor(results['cmd_path'][None, None, ...]), stack=True)
            if 'ego_lcf_feat' in results:
                results['ego_lcf_feat'] = DC(to_tensor(results['ego_lcf_feat'][None, None, ...]), stack=True)
            if 'gt_attr_labels' in results:
                results['gt_attr_labels'] = DC(to_tensor(results['gt_attr_labels']), cpu_only=False)
