...
def ts2tsemb1d(ts, num_pos_feats=128, temperature=10000):
    ...
    Args:
        pos (torch.Tensor): Input 2D position tensor.
        num_pos_feats (int, optional): Number of positional features. Default is 4096.
        temperature (int, optional): Temperature factor for positional embeddings. Default is 10000.
    ...
    dim_t = temperature ** (2 * (dim_t // 2) / num_pos_feats)
    tsemb = ts[..., 0, None] / dim_t
    tsemb = torch.stack((tsemb[..., 0::2].sin(), tsemb[..., 1::2].cos()), dim=-1).flatten(-2)
    return tsemb
