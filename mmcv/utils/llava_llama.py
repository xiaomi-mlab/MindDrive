...
class LlavaLlamaForCausalLM(LlamaForCausalLM, LlavaMetaForCausalLM):
    ...
    def __init__(self, config, use_gen_token=False, use_critical_qa=False,use_meta_action=False):
        ...
        if use_gen_token and not use_meta_action:
            weighted_mask = torch.ones(self.config.tokenizer_vocab_size + 2) 
            weighted_mask[number_tokens] = 1.0
        elif use_gen_token and use_meta_action:
            weighted_mask = torch.ones(self.config.tokenizer_vocab_size + 14)
            weighted_mask[number_tokens] = 1.0
        else:
...
def add_special_token(special_token_list, tokenizer, model):
    ...
    if num_new_tokens > 0:
        ...
        output_embed_layer = model.get_output_embeddings()
        
        input_embed_layer.weight.requires_grad = True 
        if output_embed_layer is not None: 
            output_embed_layer.weight.requires_grad = True
